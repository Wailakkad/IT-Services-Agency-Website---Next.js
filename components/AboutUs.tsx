...
export default function AboutUs() {
  const skills: Skill[] = [
    { name: "Web Development", percentage: 92, color: "from-violet-500 to-indigo-600" },
    { name: "UI/UX Design", percentage: 88, color: "from-blue-500 to-cyan-600" },
    { name: "Maintenance", percentage: 85, color: "from-emerald-500 to-teal-600" },
    { name: "Branding", percentage: 77, color: "from-amber-500 to-orange-600" },
  ];

  const stats: Stat[] = [
    { number: "2+", label: "Years of Experience", icon: <Users className="h-6 w-6" /> },
    { number: "30+", label: "Projects Completed", icon: <Code className="h-6 w-6" /> },
    { number: "20+", label: "Satisfied Clients", icon: <Users className="h-6 w-6" /> },
    { number: "15+", label: "Technologies Utilisées", icon: <Cpu className="h-6 w-6" /> },
  ];

  return (
    <div className="p-10 bg-gradient-to-b from-gray-950 to-black text-white min-h-screen">
      <Head>
        <title>About NexusDigital | Innovative Digital Solutions</title>
        <meta name="description" content="Learn about NexusDigital - your partner for innovative digital solutions that transform businesses" />
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <div className="flex flex-col items-center text-center">
            <div className="inline-block mb-6 px-4 py-1 rounded-full bg-violet-900/30 border border-violet-700/40">
              <span className="text-violet-300 text-sm font-medium">Transforming Digital Presence</span>
            </div>
            ...
            <p className="text-lg text-gray-300 max-w-3xl mb-8">
              ...
              enhance user engagement, and create meaningful connections.
            </p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
          <div className="order-2 lg:order-1">
            <div className="inline-block mb-2 px-3 py-1 rounded-full bg-violet-900/30 border border-violet-700/40">
              <span className="text-violet-300 text-sm font-medium">About NexusDigital</span>
            </div>
            ...
            <p className="text-gray-300 mb-8">
              Our collaborative approach ensures that we understand your unique challenges and goals,
              allowing us to develop tailored solutions that address your specific needs and drive 
      ...
      <section className="py-20 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto bg-gray-900/50 rounded-3xl shadow-xl shadow-violet-600/60">
        <div className="text-center mb-16">
          <div className="inline-block mb-2 px-3 py-1 rounded-full bg-violet-900/30 border border-violet-700/40">
            <span className="text-violet-300 text-sm font-medium">Our Expertise</span>
          </div>
          ...
          <p className="text-gray-300 max-w-3xl mx-auto text-lg">
            Our diverse skillset allows us to tackle any digital challenge. From front-end development 
            to immersive design experiences, we bring technical proficiency and creative vision to every project.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-16">
          {skills.map((skill, index) => (
            <div key={index} className="bg-gray-800/50 p-6 rounded-xl border border-gray-700/50 hover:border-violet-700/50 transition-all duration-300 group hover:shadow-lg hover:shadow-violet-900/10">
              <div className="flex justify-between mb-3">
                <span className="font-medium text-white group-hover:text-violet-300 transition-colors duration-300">{skill.name}</span>
                <span className="text-violet-400 font-bold">{skill.percentage}%</span>
              </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
          {stats.map((stat, index) => (
            <div key={index} className="bg-gradient-to-b from-gray-800 to-gray-900 p-8 rounded-xl text-center border border-gray-700/50 hover:border-violet-700/50 transition-all duration-300 group hover:shadow-lg hover:shadow-violet-900/20 relative overflow-hidden">
              ...
              <div className="text-4xl font-bold mb-2 text-white group-hover:text-violet-300 transition-colors duration-300">{stat.number}</div>
